from kivy.animation import Animation
from kivy.core.window import Window
from kivy.lang import Builder
from kivy.properties import ColorProperty
from kivy.uix.screenmanager import Screen
from kivy.uix.button import Button
from database.db import get_profile_by_user
from database.db import create_user
from Utils import session

Builder.load_file('kv/signup.kv')

class HoverSignupCreateButton(Button):
    bg_color = ColorProperty([0.83, 0.69, 0.22, 1])
    txt_color = ColorProperty([0.05, 0.44, 0.29, 1])

    def __init__(self, **kwargs):
        super().__init__(**kwargs)
        Window.bind(mouse_pos=self.on_mouse_pos)

    def on_mouse_pos(self, window, pos):
        if self.get_root_window() and self.collide_point(*self.to_widget(*pos)):
            Animation(bg_color=[0.08, 0.63, 0.29, 0.98], txt_color=[1, 1, 1, 1], duration=0.1, t="out_quad").start(self)
        else:
            Animation(bg_color=[0.83, 0.69, 0.22, 1], txt_color=[1, 1, 1, 1], duration=0.1, t="out_quad").start(self)

class HoverLinkButton(Button):
    def __init__(self, **kwargs):
        super().__init__(**kwargs)
        Window.bind(mouse_pos=self.on_mouse_pos)

    def on_mouse_pos(self, window, pos):
        if self.get_root_window() and self.collide_point(*self.to_widget(*pos)):
            Animation(color=[0.05, 0.44, 0.29, 1], duration=0.2, t="out_quad").start(self)
        else:
            Animation(color=[0, 0, 0, 1], duration=0.2, t="out_quad").start(self)

class SignupScreen(Screen):

    def on_enter(self):
        self.ids.signup_card.opacity = 0
        self.ids.signup_card.pos_hint = {"center_x": 0.5, "center_y": 0.4}

        entrance_anim = Animation(
            opacity=1,
            pos_hint={"center_x": 0.5, "center_y": 0.5},
            duration=0.9,
            transition="out_cubic"
        )
        entrance_anim.start(self.ids.signup_card)
    def on_leave(self):
        self.ids.password_input.text = ""
        self.ids.email_input.text = ""
        self.ids.role_input.text = ""

    def signup_user(self, instance=None):
        email = self.ids.email_input.text.strip()
        password = self.ids.password_input.text.strip()
        role = self.ids.role_input.text.strip().lower()

        if role not in ["buyer", "seller"]:
            self.ids.message_label.text = "Role must be buyer or seller"
            return

        success = create_user(email, password, role)

        if not success:
            self.ids.message_label.text = "Email already exists"
            return

        from database.db import get_user_by_email
        user = get_user_by_email(email)

        session.current_user = {
            "id": user[0],
            "email": user[1],
            "role": user[3]
        }
        if role == "buyer":
            profile = get_profile_by_user(user[0])

            if profile:
                self.manager.current = "user_dashboard"
            else:
                self.manager.current = "create_profile"

        elif role == "seller":
            self.manager.current = "create_pharmacy"